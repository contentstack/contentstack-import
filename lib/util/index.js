...
exports.validateConfig = function (config) {
  ...
  if(config.email && config.password && !config.target_stack) {
    throw new Error('Kindly provide api_token');  
  } else if(!config.email && !config.password && !config.management_token && config.source_stack) {
    ...
    throw new Error('Kindly provide Email and password for old version stack');
  }
};
