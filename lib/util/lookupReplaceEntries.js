...
module.exports = function (data, mappedUids, uidMapperPath) {
    ...
    for (var j = 0; j < len; j++) {
      if (_entry && parent[j]) {
        if (j == len - 1 && _entry[parent[j]]) {
          ...
          if (parent && j >= 0 && j < parent.length) {
          }
          var _keys = _.clone(parent).splice(eval(parseInt(j, 10) + 1), len);
  ...
  if (unmapped.length) {
    var unmappedUids = helper.readFile(
      path.join(sanitizePath(uidMapperPath), "unmapped-uids.json")
    );
  ...
  if (mapped.length) {
    var _mappedUids = helper.readFile(
      path.join(sanitizePath(uidMapperPath), "mapped-uids.json")
    );
