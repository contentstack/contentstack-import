...
var snippetsPath = path.resolve(config.data, "mapper/snippets", "uid-mapping.json");
...
var extension_uid_Replace = module.exports = function (schema) {
    for (var i in schema) {
      ...
      } else if(schema[i].data_type === 'snippet') {
        console.log("snippets+++++", schema[i])
        //schema[i]["reference_to"] = [schema[i].reference_to]
        var snippet_key_value = schema[i]["reference_to"]
        console.log("snippet_key_value", snippet_key_value)
        var snippets_data = helper.readFile(path.join(snippetsPath));
        console.log("snippets_data", snippets_data)
        if(snippets_data && snippets_data.hasOwnProperty(snippet_key_value)) {
          schema[i]["reference_to"] = snippets_data[snippet_key_value]
      }
     } else {
