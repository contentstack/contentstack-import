...
if (!fs.existsSync(snippetMapperPath)) {
      ...
      return Promise.map(snipUids, function (snipUid) {
        ...
        if (!self.snipUidMapper.hasOwnProperty(snipUid)) {
          ...
          return request(requestOption).then(function (response) {
            ...
            log.error('Snippet failed to be imported\n' + error);
            return;
          });
        } else {
          // the Snippets has already been created
          log.success('The Snippet already exists. Skipping it to avoid duplicates!');
          return;
        }
        // import 2 Snippets at a time
      }, {
        concurrency: 2
      }).then(function () {
        // snippets have imported successfully
        helper.writeFile(snippetSuccessPath, self.success);
        log.success('Snippet have been imported successfully!');
        return resolve();
      }).catch(function (error) {
        // error while importing snippets
