...
var entryMapperPath;
var environmentMapperPath;
var entryUidMapperPath;
var environmentUidMapperPath;
...
function importEntries() {
   ...
   // environmentUidMapperPath = path.join(environmentPath, 'uid-mapping.json');
...
importEntries.prototype = {
                    ...
                    return Promise.map(batches, function(batch) {
                        return Promise.map(batch, function(eUid) {
                            ...
                            if(config.entriesPublish) {
                                if(createdEntries.publish_details) {
                                self.publish(createdEntries.uid, ctUid, lang, createdEntries).then(function () {
                                    return
                                })
                                }
                            }
    ...
    publish: function(eUid, ctUid, lang, entryObj) {
        ...
        return request(requestObject).then(function() {
            log.success(chalk.green("Entry"+ eUid +" published successfully in"+ ctUid))
            return resolve();
        }).catch(function(error) {
            return reject(error);
        })    
