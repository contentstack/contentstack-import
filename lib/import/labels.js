...
importLabels.prototype = {
  start: function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var labelUids = Object.keys(self.labels);
      return Promise.map(labelUids, function (labelUid) {
        var label = self.labels[labelUid];

        // replace parentUID
        try{
          label.parent  = label.parent.map(parentLabelUID => self.labelUidMapper[parentLabelUID])
        }catch(e){
          console.log(e)
        }
        ...
        if (!self.labelUidMapper.hasOwnProperty(labelUid)) {
          ...
          return request(requestOption).then(function (response) {
            ...
            log.error('Label: \'' + label.name + '\' failed to be imported\n' + JSON.stringify(error));
            return;
          });
        } else {
          // the label has already been created
          log.success('The label: \'' + label.name +
