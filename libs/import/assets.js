...
var assetBatchLimit = (assetsConfig.hasOwnProperty('assetBatchLimit') && typeof assetBatchLimit === 'number') ? assetsConfig.assetBatchLimit : 2;
...
importAssets.prototype = {
  start: function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      return self.importFolders().then(function () {
        return Promise.map(batches, function (batch, index) {
          return Promise.map(batch, function (assetUid) {
            if (self.uidMapping.hasOwnProperty(assetUid)) {
              log.success('Skipping upload of asset: ' + assetUid + '. Its mapped to: ' + self.uidMapping[assetUid]);
            ...
            if (fs.existsSync(currentAssetFolderPath)) {
              ...
              if (config.versioning) {
                return self.uploadVersionedAssets(assetUid, currentAssetFolderPath).then(function () {
                  // assetUid and all its versions have successfully been uploaded
                  // log them onto /mapper/assets/success.json
                }).catch(function (error) {
                  log.error('Asset upload failed to import\n' + error);
                ...
                var uidContainer = {};
                var urlContainer = {};
                return self.uploadAsset(assetPath, self.assets[assetUid]).then(function () {
            ...
            log.success('Completed asset import of batch no: ' + (index + 1));
            // TODO: if there are failures, retry
            return;
          }).catch(function (error) {
            log.error('Assets upload failed to import\n' + error);
        ...
        // error while creating asset folders
        return reject(error);
      });
    });
  },
      ...
      return Promise.map(versionedAssetMetadata, function () {
        ...
        if (++counter === 1) {
          ...
          return self.uploadAsset(assetPath, assetMetadata, uidContainer, urlContainer).then(function () {
            ...
            // error while uploading specific version of asset
            throw error;
          });
        } else {
          return self.updateAsset(assetPath, assetMetadata, filesStreamed, uidContainer, urlContainer).then(function () {
            filesStreamed.push(assetMetadata.filename);
            return;
          }).catch(function (error) {
            // error while uploading specific version of asset
  ...
  updateAsset: function (assetPath, metadata, filesStreamed, uidContainer, urlContainer) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (filesStreamed && (filesStreamed.indexOf(metadata.filename) !== -1)) {
        ...
        requestOption.formData = {
          'asset[upload]': fs.createReadStream(assetPath)
        };
      }
      ...
      return request(requestOption).then(function (response) {
        ...
        // error while updating specific version of asset
        return reject(error);
      });
    });
  },
  ...
  importFolders: function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      // var pendingFolderUids = _.difference(allFolderUids, createdFolderUids);
      ...
      return Promise.map(self.folderBucket, function () {
        ...
        return request(folder).then(function (response) {
          log.success('Created folder: \'' + folder.json.asset.name + '\'');
          counter++;
          // { oldUid: newUid }
