...
// get assets object
module.exports = lookupAssets = function (data, mappedAssetUids, mappedAssetUrls, assetUidMapperPath) {
  if (!_.has(data, 'entry') || !_.has(data, 'content_type') || !_.isPlainObject(mappedAssetUids) || !_.isPlainObject(mappedAssetUrls) || typeof assetUidMapperPath !== 'string') {
  ...
  var find = function (schema, entry) {
    for (var i = 0, _i = schema.length; i < _i; i++) {
      if ((schema[i].data_type === "text" && schema[i].field_metadata && (schema[i].field_metadata.markdown || schema[i].field_metadata.rich_text_type))) {
...
function get(parent, schema, entry) {
  var _entry = entry,
    len = parent.length;
  for (var j = 0; j < len; j++) {
    try {
